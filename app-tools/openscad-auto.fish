...
set OUTPUT_EXTENSION 3mf

set SOURCE_FILE $argv[1]
set OUTPUT_FILE $SOURCE_FILE.$OUTPUT_EXTENSION
...
if test -e $OUTPUT_FILE
  set -l OUTPUT_FILE_BACKUP $OUTPUT_FILE.bak.(date "+%Y-%m-%d_%H-%M-%S").$OUTPUT_EXTENSION
...
echo "✍️ Writing new file to: "(set_color --bold)"$OUTPUT_FILE"(set_color normal)
# Set `$FONTCONFIG_PATH` to avoid https://github.com/openscad/openscad/issues/2888
set OPENSCAD_OUTPUT (
  env \
    FONTCONFIG_PATH=$(brew --prefix)/etc/fonts/ \
  openscad \
    --enable lazy-union \
    --backend Manifold \
    -o $OUTPUT_FILE \
